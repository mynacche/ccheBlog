 ...
 * @date 2013-5-20
 */
public class BlogServiceImpl extends BaseDAO {
	...
	public RespBean<Blog> add(ReqBean reqBean) {
		...
		if (blog == null) {
			respBean.setMapping("/blog/blogadd.jsp");
			return respBean;
		}
		...
		try {
			...
			int i = update(conn,
					"insert into t_blog(title,content,authorid,createtime) values (?,?,?,?)",
					blog.getTitle(), blog.getContent(), loginUser.getId(), new java.util.Date());
			if (i > 0) {
				Object[] id = query(conn, "select @@identity");

				i = update(conn, "update t_blog set blogid = ? where id=?",
						Utils.getFullId(id[0]), id[0]);
				if (i > 0) {
					tm.commitAndClose();
					respBean.setFlag(Const.SUC);
					respBean.setRespMsg(Const.BlogResult.SUC);
					respBean.setMapping(reqBean.ctxPath() + "/blog/list");
				}
			}
